...
class LLMGenerateService(
     ...
     * 获取表情包描述
     */
    fun getEmojiDescription(imageBase64: String) {
        val prompt = "这是一个表情包，使用中文简洁的描述一下表情包的内容和表情包所表达的情感"
        val resp = deepSeekRequestData.buildImageRequest(prompt, imageBase64, "user")
        val body = resp.body()
        log.info { resp }
    }
}